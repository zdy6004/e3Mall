...
function cartAdd(product_id,cart_type,opencity_id, flag,bs, obj, cfrom){
	...
	//拼装url参数，做跳转
	$.post("http://localhost:8080/cart/add/"+product_id+"?num=" + num, $("#formlogin").serialize(),function(data){
		if (data.status == 200) {
			location.href = "http://localhost:8080/front/cartSuccess";
			
		} else {
			jAlert("登录失败，原因是：" + data.msg,"失败");
		}
	});
//	location.href="http://localhost:8080/cart/add/"+product_id+".html?num=" + num;
}
